    ...
    ?>
         ...
         <div class="col-sm-12 reply-div show-comments-reply">
                   ...
                   <tr> 
                    ...
                    <td><a href="javascript:void(0)"  id="<?=$comment->id?>" post_id="<?=$comment->post_id?>" post_type="user_image" class="js-comments-reply btn-href" >Reply</a></td> 
                    <td>    
                      <?php if(is_loggedin() && is_personal()){ ?>  
                     	
                         <span>
                            <?php echo get_post_comments_count_likes($comment->post_id,$comment->id,'user_image'); ?>
                        </span>
                                       
                           <?php if(check_comments_already_given($comment->id,$this->session->userdata('user_id'),$comment->post_id,'user_image') == 0){ ?>
                        <a  href="javascript:void(0)"  id="<?=$comment->id?>" title="like" post_type="user_image" post_id="<?=$comment->post_id?>" class="set-like">
        ...
        <?php foreach($child_contents as $value){
             ...
             <div class="col-sm-9">         	
                <h6><?php echo $value['comments_author']; ?></h6>               
                   ...
                   <tr> 
                     <td>
                       <a href="javascript:void(0)"  id="<?php echo $value['id']; ?>" post_type="user_image" post_id="<?php echo $value['post_id']; ?>" class="js-comments-reply btn-href" >Reply</a>
                     </td>
                    <td>    
                      <?php if(is_loggedin() && is_personal()){ ?>  
                         <span>
                            <?php echo get_post_comments_count_likes($value['post_id'],$value['id'],'user_image'); ?>
                         </span>                                         
                           <?php if(check_comments_already_given($value['id'],$this->session->userdata('user_id'),$value['post_id'],'user_image') == 0){ ?>
                        <a  href="javascript:void(0)"  id="<?=$value['id']?>" title="like" post_type="user_image" post_id="<?=$value['post_id']?>" class="set-like">
                            <img src="<?php echo theme_url();?>/assets/images/single_thumb2.png"> 
                        </a>
                        <?php }else{ ?>
                          <img src="<?php echo theme_url();?>/assets/images/single_thumb.png"> 
