...
<div class="homebody">
          ...
          <div class="col-sm-4 homeright">
             ...
             <div class="bloghome">
             <?php foreach($random_user_blogs->result() as $blog_list){?>             
             	<div class="blog_in">
                		<?php if($blog_list->value != ''){ ?>	                
                			<img src="<?php echo base_url('uploads/users/blog/' . $blog_list->value); ?>" alt="">    
                          <?php }else{ ?>	
                          <img src="<?php echo site_url(); ?>/assets/admin/img/gallery-preview.jpg" alt=""> 
                          <?php } ?>
                <h4>
                <a href="<?=site_url();?>profile/blog/<?=$blog_list->slug?>">
					<?php echo $blog_list->title; ?></a>
                </h4></div>
...
<script type="text/javascript">
        jQuery(document).ready(function(){
            var loadUrl = '<?php echo site_url("show/load_user_comment_form/".$blog->id);?>';
            jQuery.post(
                loadUrl,
                {},
...
<script type="text/javascript">
    jQuery(document).ready(function(){ 
	   ...
	   //load all blog.
	    var loadUrl = '<?php echo site_url("show/load_all_user_blog_comments_view/".$blog->id);?>';
        jQuery.post(
            loadUrl,
            {},
            function(responseText){
