 ...
 <h3>Blogs</h3>
 	   <?php if($user_id == $this->session->userdata('user_id')){ ?>
            <div class="add-more-btn btn-green"><i class="glyphicon glyphicon-plus"></i>        	
                <a href="<?php echo site_url('admin/add_blog/0');?>" class="">Add More Blog Post</a>
            </div>        
        <?php } ?>
...
$i++;
 ...
 ?>
   <li>  
   	  <?php if($blog->value != ''){ ?>	
   	 	<img src="<?php echo base_url('uploads/users/blog/'.$blog->value); ?>">
     <?php }else{ ?>
    	   <img src="<?php echo base_url('assets/admin/img/gallery-preview.jpg'); ?>">
     <?php } ?>	
     <h4>
     	<a href="<?php echo site_url().'profile/blog/'.$blog->slug;?>"><?php echo $blog->title;?></a>
     </h4>
</li>    
<?php } ?>
</ul>
...
<div class="show-video pos-relative">
 		<h3> Blogs </h3>   
            <div class="add-more-btn btn-green"><i class="glyphicon glyphicon-plus"></i>        	
                <a href="<?php  echo site_url('admin/add_blog/0');?>" class="">Add Blog Post</a>
            </div> 
     </div>       
        <?php } ?>
<?php } ?>
