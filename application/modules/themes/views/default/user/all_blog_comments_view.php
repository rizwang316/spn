               ...
               <tr> 
                 <td>    
                  <?php if(is_loggedin() && is_personal()){ ?>  
                       ...
                       <?php if(check_comments_already_given($comment->id,$this->session->userdata('user_id'),$comment->post_id,'user_blog') == 0){ ?>
                    ...
                    <?php } ?>                     
                    <span>
                    	<?php echo get_post_comments_count_likes($comment->post_id,$comment->id,'user_blog'); ?>
                    </span>
                  <?php }else{ ?> 
         ...
         <div class="col-sm-9">         	
            ...
            <div><?php echo date('g:ia \o\n l jS F Y', $value['comment_date']);?></div>
               ...
               <tr> 
               	<td>    
                  <?php if(is_loggedin() && is_personal()){ ?>                   
                       <?php if(check_comments_already_given($value['id'],$this->session->userdata('user_id'),$value['post_id'],'user_blog') == 0){ ?>
                    ...
                    <?php } ?>                     
                    <span>
                    	<?php echo get_post_comments_count_likes($value['post_id'],$value['id'],'user_blog'); ?>
                    </span>
                  <?php }else{ ?> 
                   <a  href="javascript:login_alert()">
                    	<img src="<?php echo theme_url();?>/assets/images/single_thumb2.png"> 
	...
	<script type="text/javascript">
            ...
            jQuery(document).ready(function(){
             jQuery('.js-comments-reply').on('click',function(e){
                ...
                var loadUrl = '<?php echo site_url()?>/show/load_user_comment_form_reply/'+post_id+'/'+id;
                jQuery.post(
                    loadUrl,
                    {},
