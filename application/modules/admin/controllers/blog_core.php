...
class Blog_core extends CI_Controller {
	...
	public function view_all_reviews($page='0',$post_id='')
	{
		...
		$value['reviews']  	= get_all_comments($post_id,'blog');
		$data['title'] 		= lang_key('all_reviews');
		$data['content'] 	= load_admin_view('blog/all_reviews_view',$value,TRUE);
	...
	public function deletecomment($page='0',$post_id='',$id='',$confirmation='')
	{
		...
		if($confirmation=='')
		{
			$data['content'] = load_admin_view('confirmation_view',array('id'=>$id,'url'=>site_url('admin/blog/deletecomment/'.$page.'/'.$post_id)),TRUE);
			load_admin_view('template/template_view',$data);
		}
		else
		{
			...
			redirect(site_url('admin/blog/view_all_reviews/'.$page.'/'.$post_id));
		}		
	}
