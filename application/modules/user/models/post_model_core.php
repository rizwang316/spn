...

{
	...
	function count_post_by_category_id($cat_id)
	{
		...
		$this->db->where("(category_post_relationships.category_id = $cat_id)");		
	...
	function get_category_posts($limit=6,$category_id='') {
		...
		$this->db->join('category_post_relationships', 'category_post_relationships.post_id = posts.id');
		$this->db->where('category_post_relationships.category_id', $category_id);
		//
		$this->db->where('posts.status',1);	
		$this->db->limit($limit);
	...
	{
		$query = $this->db->get_where('spn_category_post_relationships',array('post_id'=>$id));
		if($query->num_rows()<=0)
		{
			echo 'Invalid post id';
			die;
