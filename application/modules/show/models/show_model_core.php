...

{
    ...
    function get_all_active_blog_posts_by_range($start,$limit='',$sort_by='',$sort='desc',$type="all")
    {
        ...
        if($type!='all'){
			// get category id.	
			$cate_id = get_blog_category_id_by_slug($type);
			$this->db->select('blog.*');
			$this->db->where('category_blog_relationships.category_id',$cate_id->id);
			$this->db->join('category_blog_relationships', 'category_blog_relationships.post_id = blog.id');			
		}
    ...
    function get_advanced_search_result($data,$start = '0',$limit = '10') {
        ...
        {
			$this->db->select('category_post_relationships.category_id' );	
			$this->db->select('posts.*' );			 		
			$this->db->join('category_post_relationships', 'category_post_relationships.post_id = posts.id');
			$this->db->where('category_post_relationships.category_id', $data['category']);
		//	$this->db->group_by('posts.id');
			//$this->db->select('category_post_relationships.post_id');
	...
	function get_recent_review()
	{
		// post.
		//$this->db->select('posts.category_id' );	
		$this->db->select('review.*' );			 		
		$this->db->join('posts', 'review.post_id = posts.id');
