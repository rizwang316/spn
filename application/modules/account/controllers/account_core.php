...
class Account_core extends CI_Controller {
...

	{
...

		{
			...
			// check user name.
			if($query->num_rows() == 0){
				$query = $this->auth_model->check_login_username($this->input->post('useremail'),$this->input->post('password'),'result');
			}
			...
			{				
...

				{
					...
					// set data for user login checking
					//$this->onlineusers->set_data(array('username'=>$row->user_name,'id'=>$row->id), 1);				
					//$this->onlineusers->set_data($row->user_name,$row->id);				
	...
	function register()
	{
		...
		$this->form_validation->set_rules('terms_conditon',lang_key('terms_and_condition'),'required|xss_clean');
		...
		else
		{
			...
			$userdata['user_type']	= 3; //3 = Individual users
			...
			add_user_meta($user_id,'about_me',$this->input->post('about_me'));			
	...
	{
		$data['content'] 	= load_view('success_msg_view','',TRUE,$this->active_theme);
		load_template($data,$this->active_theme);
	}
	...
	public function send_notification_mail($msg)
	{
		...
		$this->email->from('arfan0975@gmail.com', 'Paypal Test');
		$this->email->to('arfan0975@gmail.com');
	...
	public function send_confirmation_email($data=array('username'=>'sc mondal','useremail'=>'arfan0975@gmail.com','confirmation_key'=>'1234'))
	{
		$val = $this->get_admin_email_and_name();
		$admin_email = $val['admin_email'];
		$admin_name  = $val['admin_name'];
	...
	public function confirm($email='',$code='')
	{
		...
		if($res==TRUE)
		{
			$user_data = $this->auth_model->get_userdata_by_email($email);			
			$this->auth_model->active_business_after_active_account($user_data['id']);
	...
	{		
		...
		$this->form_validation->set_rules('first_name',	lang_key('first_name'), 'required|xss_clean');
		$this->form_validation->set_rules('last_name',	lang_key('last_name'), 	'required|xss_clean');
		$this->form_validation->set_rules('gender',		lang_key('gender'), 			'xss_clean');
	    $this->form_validation->set_rules('username', 	lang_key('username'), 'required|callback_username_check|xss_clean');
        $this->form_validation->set_rules('company_name',lang_key('company_name'), 	'xss_clean');
        $this->form_validation->set_rules('phone',lang_key('phone'), 'xss_clean');
        $this->form_validation->set_rules('useremail',	lang_key('user_email'), 		'required|valid_email|xss_clean|callback_useremail_check');
		...
		else
		{
			if(is_loggedin()){
			 ...
			 if(!is_loggedin()){
				$userdata['user_type']	= 2; //2 = business
				$userdata['first_name'] = $this->input->post('first_name');
				$userdata['last_name'] 	= $this->input->post('last_name');
			...
			**************************/
			...
			else
			{
				...
				if(!is_loggedin()){	
					redirect(site_url('account/success_msg'));			
				}else{
