...
class Auth_model_core extends CI_Model 
{
	...
	function check_login_username($user_email,$password,$return_as='num_rows')
	{
		$password = md5($password);
	...
	function active_business_after_active_account($user_id)
	{
		$query = $this->db->get_where('posts',array('created_by'=>$user_id));
		if($query->num_rows()>0)
		{
			$data['status'] = 1;			
			$this->db->update('posts',$data,array('created_by'=>$user_id));
