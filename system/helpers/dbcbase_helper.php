...
if ( ! function_exists('get_user_type_by_user_id'))
{
	function get_user_type_by_user_id($id)
	{
		 $CI = get_instance();
        $CI->load->database();
        $query = $CI->db->get_where('users',array('id'=>$id));
...
if ( ! function_exists('is_personal'))
{
	...
	{
		$CI = get_instance();
		if($CI->session->userdata('user_name')!='' && $CI->session->userdata('user_type')==3)
			return TRUE;		
		else
			return FALSE;
	}
...
{
	function paginationConfig($url,$total_rows,$segment,$per_page=10)
	{
		   // paginatoin
		   $CI = get_instance();
		   $CI->load->library('pagination');
...
// get post by id.
if ( ! function_exists('get_post_data_by_id'))
{
 function get_post_data_by_id($id)
	{
		 $CI = get_instance();
		$query = $CI->db->get_where('posts',array('id'=>$id,'status !='=>0));
		if($query->num_rows()>0)
		{
			return $query->row();
		}
